...
public class DogRecordRepoImpl extends SQLiteOpenHelper implements DogRecordRepository {
    public static final String TABLE_NAME = "dogRecord";
    ...
    public DogRecordRepoImpl(Context context) {
    ...
    @Override
    public DogRecord findById(Long id) {
        ...
        if (cursor.moveToFirst()) {
            final DogRecord animlRec = new DogRecord.Builder()
    ...
    @Override
    public DogRecord save(DogRecord entity) {
        try {
            open();
        ...
        DogRecord insertedEntity = new DogRecord.Builder()
                .copy(entity)
                .id(new Long(id))
                .build();
    ...
    @Override
    public DogRecord update(DogRecord entity) {
        try {
            open();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    @Override
    public DogRecord delete(DogRecord entity) {
        try {
            open();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    @Override
    public Set<DogRecord> findAll() {
        SQLiteDatabase db = this.getReadableDatabase();
        Set<DogRecord> elections = new HashSet<>();
        try {
            open();
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        if (cursor.moveToFirst()) {
            do {
                final DogRecord election = new DogRecord.Builder()
                        .id(cursor.getLong(cursor.getColumnIndex(COLUMN_ID)))
                        .arrivalDate(cursor.getString(cursor.getColumnIndex(COLUMN_ARRIVALDATE)))
                        .leavingDate(cursor.getString(cursor.getColumnIndex(COLUMN_LEAVINGDATE)))
                        .build();
